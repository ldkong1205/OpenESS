...
class MaskClipHead(nn.Module):
    ...
    def forward(self, inputs):
        ...
        logists = self.cls_seg(feat)
        # if not self.training:
        #     output = self.refine_output(output, k)

        return image_feats, logists
...
class maskClipFeatureExtractor(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = MaskClipHead(
            text_embeddings_path=text_embeddings_path,
            visual_projs_path=visual_projs_path,
        ...
        self.checkpoint = maskclip_checkpoint
    ...
    def forward(self, img):
        ...
        logists = resize(
            input=logists,
            size=(img.shape[2],img.shape[3]),   #Todo
            mode='bilinear',
            align_corners=self.align_corners)

        return logists #, feats
        # return feat, seg_pred, posibi
