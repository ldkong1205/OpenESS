...
from predictor_ddd17 import VisualizationDemo
...
if __name__ == "__main__":
    ...
    image_paths = glob.glob(args.root_folder + "/*/images_aligned/*.png", recursive=True)
    image_paths = sorted(image_paths)
    

    for i_path in tqdm(image_paths, desc="Generating pseudo labels"):
        path = Path(i_path)
        dir_name = i_path.split('/')[-3]   
        image_name = path.name      
        save_path = args.root_folder / dir_name / "pl_fcclip_rgb" / image_name.replace("img","segmentation") 
        save_path.parent.mkdir(parents=True, exist_ok=True)

        img = read_image(path, format="BGR")
        predictions, visualized_output = demo.run_on_image(img)

        user_to_ddd17 = torch.tensor([
                0, 0,
                1, 1,
                2, 2, 2, 2, 2, 2, 2,
                3, 3, 3, 3, 3,
        ...
        num_ddd17_classes = 6
        logits = torch.full((num_ddd17_classes, H, W), float('-inf'), device=sem_seg.device)

        for user_idx in range(len(user_to_ddd17)):
            ddd17_idx = user_to_ddd17[user_idx]
            logits[ddd17_idx] = torch.maximum(logits[ddd17_idx], sem_seg[user_idx])

        pseudo_mask = logits.argmax(0)  
